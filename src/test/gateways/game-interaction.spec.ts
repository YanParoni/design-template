...
describe('GameInteractionGateway', () => {
  ...
  it('should create a game interaction', async () => {
    const interactionDto: CreateInteractionDto = { gameId: 'game-id', liked: true, played: false };
    fetchHttpClientMock.post.mockResolvedValue({ data: { id: 'interaction-id' }, error: null });

    const response = await gameInteractionGateway.createInteraction(interactionDto);
  ...
  it('should fetch user interactions', async () => {
    fetchHttpClientMock.get.mockResolvedValue({ data: [{ id: 'interaction-id' }], error: null });
    const interactions = await gameInteractionGateway.getUserInteractions('user-id');
